...
class InterFeatureDownsample(nn.Module):
    ...
    def forward(self, x, fm):
        ...
        x = F.interpolate(x, size=fm.shape[2::], mode='bilinear', align_corners=True)
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=19, bins=(1, 2, 3, 6), norm_layer=nn.BatchNorm2d, bn_eps=1e-5,
        ...
        self.ppm = PPM(int(np.rint(512 * alpha)), int(int(np.rint(512 * alpha)) / len(bins)), bins, norm_layer)
    ...
    def forward(self, x):
        ...
        x = self.fusion(self.conv_low(blocks[-2]), self.conv_high(blocks[-1])) # the channel size is too small, now change to int(np.rint(512 * alpha))
        x = self.context(x) # output channel is int(np.rint(512 * alpha))
        # x = F.interpolate(x, scale_factor=4, mode='bilinear', align_corners=True) # feature from self-attention
        #print(x.shape, 'xxxx',blocks[-1].shape)
        # x = self.head(torch.cat((blocks[-1],x),dim=1))
        # blocks.append(x)

        #now comes ld alogrithm on small resolution feature maps
        camap = F.sigmoid(self.depthwise_conv(x)) #we obtain int(np.rint(512 * alpha))-channel feature map
        # map = F.sigmoid(map)
        x = camap*x + x
...
if __name__ == "__main__":
    ...
    alpha = config.alpha
    img = torch.randn(2, 3, int(np.rint(256 * alpha)), int(np.rint(512 * alpha)))
    start = time.time()
    for i in range(10):
      output = model(img)
