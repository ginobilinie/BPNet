...
#from engine.evaluator1 import Evaluator
from engine.logger import get_logger
from seg_opr.metric import hist_info, compute_score
from datasets.cityscapes import Cityscapes
from network import FPNet
#from network_mobile_v2 import FPNet
...
class SegEvaluator(Evaluator):
    def func_per_iteration(self, data, device):
        ...
        label = data['label']
        name = data['fn']
        #print(img.shape,'123456',label.shape,'654321',name)
        pred = self.whole_eval(img, (config.image_height, config.image_width), device)
        #pred = self.sliding_eval(img, config.eval_crop_size, config.eval_stride_rate, device)
        #pred = self.sliding_rectangular_eval(img, config.eval_crop_height, config.eval_crop_width,config.eval_stride_rate, device)
...
if __name__ == "__main__":
    ...
    network = FPNet(config.num_classes, is_training=False,
                      criterion=None, ohem_criterion=None, dice_criterion=None, norm_layer=nn.BatchNorm2d, alpha= config.alpha)
