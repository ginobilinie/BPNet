...
class DilatedParallelResidualBlockB(nn.Module):
    ...
    def __init__(self, nIn, nOut, add=True):
        ...
        '''
        super(DilatedParallelResidualBlockB, self).__init__()
        n = int(nOut / 5)
        n1 = nOut - 4 * n
        self.c1 = C(nIn, n, 1, 1)
...
'''
    seperable conv: depth-wise conv+point-wise conv
'''
def ConvBnRelu_DW(in_planes, out_planes, ksize=3, stride=1, pad=0, dilation=1, groups=1, norm_layer=nn.BatchNorm2d,
                  has_bn=True, bn_eps=1e-5,
                 has_relu=True, inplace=True, has_bias=False):
    return nn.Sequential(
        nn.Conv2d(in_planes, in_planes, ksize, stride, pad, groups=in_planes, bias=False),
...
'''
    seperable conv: depth-wise conv+point-wise conv
'''
def ConvBn_DW(in_planes, out_planes, ksize=3, stride=1, pad=0, dilation=1, groups=1, norm_layer=nn.BatchNorm2d,
              has_bn=True, bn_eps=1e-5,
                 has_relu=True, inplace=True, has_bias=False):
    return nn.Sequential(
        nn.Conv2d(in_planes, in_planes, ksize, stride, pad, groups=in_planes, bias=False),
...
'''
    seperable conv: depth-wise conv+point-wise conv
'''
def Conv_DW(in_planes, out_planes, ksize=3, stride=1, pad=0, dilation=1, groups=1, norm_layer=nn.BatchNorm2d,
              has_bn=True, bn_eps=1e-5,
                 has_relu=True, inplace=True, has_bias=False):
    return nn.Sequential(
        nn.Conv2d(in_planes, in_planes, ksize, stride, pad, groups=in_planes, bias=False),
