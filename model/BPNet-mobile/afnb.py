...
from blocks import ConvBnRelu, ConvBnRelu_DW, ConvBn_DW,Conv_DW
...
class _SelfAttentionBlock(nn.Module):
    ...
    '''
    def __init__(self, low_in_channels, high_in_channels, key_channels, value_channels, out_channels=None, scale=1, norm_layer=nn.BatchNorm2d, psp_size=(1,3,6,8)):
        ...
        self.f_key = ConvBnRelu(self.in_channels, self.key_channels, 1, 1, 0, has_bn=True, norm_layer=norm_layer, has_relu=True, has_bias=False)

        self.f_query = ConvBnRelu(high_in_channels, self.key_channels, 1, 1, 0, has_bn=True, norm_layer=norm_layer, has_relu=True, has_bias=False)
...
class AFNB(nn.Module):
    ...
    def __init__(self, low_in_channels, high_in_channels, out_channels, key_channels, value_channels, dropout,
                 sizes=([1]), norm_layer=nn.BatchNorm2d,psp_size=(1,3,6,8)):
        ...
        self.conv_bn_dropout = nn.Sequential(
            ConvBnRelu(out_channels + high_in_channels, out_channels, 1, 1, 0, has_bn=True, norm_layer=norm_layer, has_relu=True,
                       has_bias=False),
