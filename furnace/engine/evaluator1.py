...
class Evaluator(object):
    ...
    # slide the window (only square) to evaluate the image
    ...
    # slide the window with height and width (instead of only square) to evaluate the image
    def sliding_rectangular_eval(self, img, crop_height, crop_width, stride_rate, device=None):
        ori_rows, ori_cols, c = img.shape
        processed_pred = np.zeros((ori_rows, ori_cols, self.class_num))
        ...
        for s in self.multi_scales:
            img_scale = cv2.resize(img, None, fx=s, fy=s,
                                   interpolation=cv2.INTER_LINEAR)
            new_rows, new_cols, _ = img_scale.shape
            processed_pred += self.scale_rectangular_process(img_scale,
                                                 (ori_rows, ori_cols),
                                                 crop_height, crop_width, stride_rate, device)
