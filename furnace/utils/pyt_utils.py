...
'''
To use SE loss, we have to transform labels in a feature map to 
vector labels
input: feature map (BxHxW)
output: feature vector (Bxclasses)
...
def _get_vector_label_from_map(target, num_class):
    ...
    for i in range(batch):
        hist = torch.histc(target[i].cpu().data.float(),
                bins=num_class, min=0, max=num_class-1)
        vect = hist>0
        mvect[i] = vect
    return mvect
